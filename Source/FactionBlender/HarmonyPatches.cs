...
internal class HarmonyPatches
{
    ...
    {
        if (p?.def?.thingClass != null && p is { skills: { }, workSettings: { } })
        {
            return true;
        }
    ...
    [HarmonyPrefix]
    private static bool GenerateCarriers_Override(PawnGroupMakerParms parms, PawnGroupMaker groupMaker,
        List<Thing> wares, List<Pawn> outPawns)
    {
        float MassTotaler(Thing t)
        {
            return t.stackCount * t.GetStatValue(StatDefOf.Mass);
        }

        var list = wares.Where(t => t is not Pawn).ToList();
        list.SortByDescending(MassTotaler);

        var ttlMassThings = list.Sum(MassTotaler);
        var ttlCapacity = 0f;
        var ttlBodySize = 0f;
        var numCarriers = 0;

        var carrierKinds = groupMaker.carriers.Where(p =>
            parms.tile == -1 || Find.WorldGrid[parms.tile].biome.IsPackAnimalAllowed(p.kind.race));

        var kind = carrierKinds.RandomElementByWeight(x => x.selectionWeight).kind;

        // No slow or small juveniles
        bool Validator(Pawn p)
        {
            return p.ageTracker.CurLifeStage.bodySizeFactor >= 1 && p.GetStatValue(StatDefOf.MoveSpeed) >=
                p.kindDef.race.GetStatValueAbstract(StatDefOf.MoveSpeed);
        }

        // 50/50 chance of uniform carriers (like vanilla) or mixed carriers
        var mixedCarriers = Rand.RangeInclusive(0, 1) == 1;
        ...
        // size.
        var carrierPawns = new List<Pawn>();
        ...
        {
            var validPawns = carrierPawns.FindAll(p =>
                !MassUtility.WillBeOverEncumberedAfterPickingUp(p, thing, thing.stackCount));

            if (validPawns.Count != 0)
            {
                validPawns.RandomElement().inventory.innerContainer.TryAdd(thing);
            }
            else if (thing.stackCount > 1)
            {
                // No carrier can handle the full stack; split it up
                var countLeft = thing.stackCount;
                var c = 0; // safety counter (while loops can be dangerous)
                while (countLeft > 0)
                {
                    ...
                    {
                        var pawn = validPawns.RandomElement();
                        var countToAdd = Mathf.Min(MassUtility.CountToPickUpUntilOverEncumbered(pawn, thing),
                            countLeft);
                        countLeft -= pawn.inventory.innerContainer.TryAdd(thing, countToAdd);
                    }
                    else
                    {
                        ...
                        var splitCount = Mathf.FloorToInt((float)countLeft / carrierPawns.Count);
                        if (splitCount > 0)
                        {
                            carrierPawns.ForEach(p => p.inventory.innerContainer.TryAdd(thing, splitCount));
                        ...
                        while (countLeft > 0)
                        {
                            carrierPawns.MaxBy(MassUtility.FreeSpace).inventory.innerContainer.TryAdd(thing, 1);
                            countLeft--;
                        }
            ...
            else
            {
                ...
                carrierPawns.MaxBy(MassUtility.FreeSpace).inventory.innerContainer.TryAdd(thing);
            }
        }
    ...
    private static void GenerateWorkingPossibleApparelSetFor_Patch(Pawn pawn, List<ThingStuffPair> ___allApparelPairs)
    {
        ...
        var CanUseStuffMethod = AccessTools.Method(typeof(PawnApparelGenerator), "CanUseStuff");

        var reqApparel = pawn.kindDef.apparelRequired;
        foreach (var thingDef in reqApparel)
        {
            var pairs = ___allApparelPairs.Where(
                pa => pa.thing == thingDef
            );
            ...
            var logMsg =
                    $"Found an apparelStuffFilter/stuffCategories conflict for required apparel {thingDef} while generating apparel for {pawn.kindDef.defName}; "
                ;

            var factionFilter = pawn.Faction.def.apparelStuffFilter;
            var factionName = pawn.Faction.def.defName;
            if (factionFilter != null)
            {
                var stuffCategories = thingDef.stuffCategories;
                var examplePair = pairs.RandomElementByWeight(pa => pa.Commonality);

                if (stuffCategories == null && examplePair is { stuff: { } })
                {
                ...
                {
                    logMsg =
                        $"{logMsg}adding extra stuffCategories to {factionName}'s apparelStuffFilter: {string.Join(", ", stuffCategories)}";

                    foreach (var sc in stuffCategories)
                    {
    ...
    private static void TryGenerateNewPawnInternal_Patch(ref PawnGenerationRequest request)
    {
        ...
        if (Find.FactionManager.RandomRoyalFaction() != null)
        {
            return;
        }
    ...
    public static class BiomeDef_IsPackAnimalAllowed_Patch
    {
        ...
        [HarmonyPostfix]
        private static void IsPackAnimalAllowed_Patch(BiomeDef __instance, ref bool __result,
            List<ThingDef> ___allowedPackAnimals, ThingDef pawn)
        {
            ...
            var biomeName = __instance.defName;
            if (!biomeMinMaxTemp.ContainsKey(biomeName))
            {
                ...
                {
                    Log.Warning(
                        $"[FactionBlender] Unrecognized biome {biomeName}.  Accepting all pack animals for traders here, which might cause some to freeze/burn to death, if the biome is hostile.  Ask the FB dev to include the biome in the static min/max temp list."
                    );
            ...
            var minTemp = minMaxTemp[0];
            var maxTemp = minMaxTemp[1];

            if (!(pawn.GetStatValueAbstract(StatDefOf.ComfyTemperatureMin) <= minTemp) ||
                !(pawn.GetStatValueAbstract(StatDefOf.ComfyTemperatureMax) >= maxTemp))
            {
                return;
            }
    ...
    public static class MindStateTick_Patch
    {
        ...
        private static void Patch(Pawn_MindState __instance)
        {
            ...
            var job = pawn.CurJob;
            if (job == null || !TryCheckFriendlyFaction(pawn, job.targetA.Thing) || job.def != JobDefOf.AttackMelee &&
                job.def != JobDefOf.AttackStatic && job.def != JobDefOf.Hunt &&
                job.def != JobDefOf.Ignite &&
                job.def != JobDefOf.Ingest && job.def != JobDefOf.Kidnap && job.def != JobDefOf.PredatorHunt &&
            ...
            // Warn about it
            var tPawn = job.targetA.Thing as Pawn;
            Base.Instance.ModLogger.Warning(
                $"Friendly faction attack ({job.def}) between {pawn} and {tPawn}.  The pawn type may need to be blacklisted in the Faction Blender configuration.  (Race: {pawn.kindDef.race.defName}, defaultFactionType: {pawn.kindDef.defaultFactionType})"
            );
        ...
        internal static bool TryCheckFriendlyFaction(Pawn pawn, Thing target)
        {
            ...
            return !pawn.Faction.HostileTo(tPawn.Faction) && !GenHostility.IsActiveThreatTo(tPawn, pawn.Faction);
        }
    }
    ...
    public static class GenerateAncientsPatches
    {
        private static Pawn GenerateBaseAncient(ThingSetMaker_MapGen_AncientPodContents instance)
        {
            ...
            var faction = Find.FactionManager.AllFactionsListForReading.FirstOrDefault(f =>
                f.def.defName == "FactionBlender_Civil");

            if (faction == null)
            {
                faction = Find.FactionManager.AllFactionsListForReading.FirstOrDefault(f =>
                    f.def.defName == "FactionBlender_Pirate");
            }

            if (faction == null)
            {
                Find.FactionManager.TryGetRandomNonColonyHumanlikeFaction(out faction, true, true, allowTemporary: true,
                    minTechLevel: TechLevel.Spacer);
            }

            if (faction == null)
            {
                Find.FactionManager.TryGetRandomNonColonyHumanlikeFaction(out faction, true, true,
                    allowTemporary: true);
            }

            // 20% chance of animals and others
            PawnKindDef pawnKind = null;
            var allowNonHuman = Rand.Range(1, 5) == 1;
            for (var i = 0; i < 10; i++)
            {
                pawnKind = faction != null
                    ? faction.RandomPawnKind()
                    : PawnKindDefOf.AncientSoldier; // fallback of last resort

                if (pawnKind.RaceProps.baseBodySize >= 3)
                {
                    continue; // maybe no dinosaurs in cryptosleep caskets?
                }

                if (allowNonHuman || pawnKind.RaceProps.Humanlike)
                {
                    break;
                }
            }
            ...
            var giveLootMethod = AccessTools.Method(typeof(ThingSetMaker_MapGen_AncientPodContents),
                "GiveRandomLootInventoryForTombPawn");
        ...
        [HarmonyPrefix]
        private static bool GenerateFriendlyAncient_Override(ThingSetMaker_MapGen_AncientPodContents __instance,
            ref Pawn __result)
        {
            // Skip this method (if the config is disabled)
            if (!(SettingHandle<bool>)Base.Config["EnableMixedAncients"])
            {
                return true;
            }
        ...
        [HarmonyPrefix]
        private static bool GenerateIncappedAncient_Override(ThingSetMaker_MapGen_AncientPodContents __instance,
            ref Pawn __result)
        {
            // Skip this method (if the config is disabled)
            if (!(SettingHandle<bool>)Base.Config["EnableMixedAncients"])
            ...
            var pawn = GenerateBaseAncient(__instance);
        ...
        [HarmonyPrefix]
        private static bool GenerateAngryAncient_Override(ThingSetMaker_MapGen_AncientPodContents __instance,
            ref Pawn __result)
        {
            // Skip this method (if the config is disabled)
            if (!(SettingHandle<bool>)Base.Config["EnableMixedAncients"])
        ...
        [HarmonyPrefix]
        private static bool GenerateHalfEatenAncient_Override(ThingSetMaker_MapGen_AncientPodContents __instance,
            ref Pawn __result)
        {
            // Skip this method (if the config is disabled)
            if (!(SettingHandle<bool>)Base.Config["EnableMixedAncients"])
        ...
        [HarmonyPrefix]
        private static bool GenerateScarabs_Override(ref List<Thing> __result)
        {
            // Skip this method (if the config is disabled)
            if (!(SettingHandle<bool>)Base.Config["EnableMixedAncients"])
            {
            ...
            var pawnKind = DefDatabase<PawnKindDef>.AllDefsListForReading.Where(p =>
                p.isFighter && !p.RaceProps.Humanlike &&
                p.combatPower is >= 20 and <= 240 &&
                (
            ...
            var cpLimit = Rand.Range(120, 240);
            var ttlCP = 0;
            while (ttlCP < cpLimit)
            {
