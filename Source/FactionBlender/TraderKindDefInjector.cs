...
public class TraderKindDefInjector
{
    ...
    public static void InjectTraderKindDefsToFactions(List<FactionDef> FB_Factions)
    {
        ...
        foreach (var FB_Faction in FB_Factions.Where(fd => !fd.permanentEnemy))
        {
            var shortDefName = FB_Faction.defName.Replace("FactionBlender", "FB");
            var tkdPrefix = $"{shortDefName}_Caravan";

            var FB_TraderKinds = FB_Faction.caravanTraderKinds;

            var traderKindDefs = DefDatabase<FactionDef>.AllDefs
            ...
            for (var i = 0; i < FB_TraderKinds.Count; i++)
            {
                var traderKind = FB_TraderKinds[i];

                var curLabel = traderKind.label?.ToLower();
                if (curLabel == null)
                {
                    continue;
                }

                var newLabel = newTraderLabel.ContainsKey(curLabel) ? newTraderLabel[curLabel] : null;
                if (newLabel == null)
                {
                    continue;
                }
            ...
            {
                var curLabel = traderKind.label?.ToLower();
                var newLabel = curLabel != null && newTraderLabel.ContainsKey(curLabel)
                    ? newTraderLabel[curLabel]
                    : null;
                var lm = FB_TraderKinds.FirstIndexOf(tkd =>
                    tkd.label?.ToLower() is { } lcLabel && (lcLabel == curLabel || lcLabel == newLabel)
                );

                // If we somehow missed a dupe, skip it
                if (FB_Faction.caravanTraderKinds.Contains(traderKind))
                {
                    continue;
                }
                ...
                if (lm >= 0)
                {
                    ...
                    {
                        var newTraderKind = CopyTraderKindDef(labelMatchTKD, tkdPrefix, labelMatchTKD.LabelCap);
                ...
                {
                    var newTraderKind = CopyTraderKindDef(traderKind, tkdPrefix, newLabel, newLabel);
                ...
                {
                    FB_Faction.caravanTraderKinds.Add(traderKind);
                }
            }
            ...
            FB_Faction.visitorTraderKinds.AddRange(
                DefDatabase<FactionDef>.AllDefs.Where(f => !FB_Factions.Contains(f))
                    .SelectMany(f => f.visitorTraderKinds)
            );

            /* Base Trader Kinds */

            // The base starts with the outlander base StockGenerators, for the essentials
            var baseTraderKind = CopyTraderKindDef(DefDatabase<TraderKindDef>.GetNamed("Base_Outlander_Standard"),
                shortDefName, "Base Trade Standard");
            FB_Faction.baseTraderKinds[0] = baseTraderKind;
            baseTraderKind.requestable = false;

            var otherTraderKinds = DefDatabase<FactionDef>.AllDefs.Where(f => !FB_Factions.Contains(f))
                .SelectMany(f => f.baseTraderKinds).ToList();
            var otherStockGenerators = otherTraderKinds.SelectMany(tkd => tkd.stockGenerators).ToList();
            ...
            double avgBaseGeneratorCount =
                otherTraderKinds.Sum(tkd => tkd.stockGenerators.Count) / (float)otherTraderKinds.Count;
            var targetGeneratorCount = (int)Math.Ceiling(avgBaseGeneratorCount * 2);
            ...
            if (newGeneratorList.Count > targetGeneratorCount)
            {
                newGeneratorList = newGeneratorList.GetRange(0, targetGeneratorCount);
            }
            ...
            baseTraderKind.stockGenerators.AddRange(
                otherStockGenerators.Where(sg =>
                    sg is StockGenerator_BuyExpensiveSimple or StockGenerator_BuySingleDef
                        or StockGenerator_BuySlaves or StockGenerator_BuyTradeTag
                )
            );
            ...
            {
                baseTraderKind.stockGenerators =
                    baseTraderKind.stockGenerators.Where(sg => sg is not StockGenerator_Slaves).ToList();
            }
        }
    }

    public static TraderKindDef CopyTraderKindDef(TraderKindDef origTraderKind, string defNamePrefix, string labelBase,
        string newLabel = null)
    {
        var newDefName = $"{defNamePrefix}_{GenText.ToTitleCaseSmart(labelBase).Replace(" ", "_")}";
    ...
    {
        if (newTraderKind.orbital)
        {
            return; // pendatic sanity check
        }
